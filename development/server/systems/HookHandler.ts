...
export class HookHandler {
	...
	private _eventHooks:astd.HMap<string, astd.IHMap<string, Function>>;
	...
	public triggerEvent(eventName:string, ...args:any[]):void {
		...
		if (!eventName.toLowerCase().includes("update"))   // Do not debug any events ending in "update"
		...
		if (this._eventHooks.containsKey(eventName)) {
			...
			let callbacks:astd.IHMap<string, Function> = this._eventHooks.get(eventName);
			...
			for (let hookUIdIter in callbacks.getKeys()) {
				...
				if (!eventName.toLowerCase().includes("update"))
